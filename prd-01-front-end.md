# Polly Prediction Partners - Product Requirements
...
- **Priority:** we keep making agents that work, but with an ugly UI. So this time, let's make a pretty UI then focus on building agents and trading data bases.
- [ ] Javascript Vanilla
- [ ] Tailwind CSS
- [ ] HTML/HTMX
- [ ] **Pretty** must be pretty, hyper-polished, detailed, reactive, and responsive UI/UX
- [ ] Components refresh via HTMX once back end starts getting hooked up.
- [ ] Must be confirmed with Main User UI/UX WebPage pasees 90% of requirements.
- [ ] Must be confirmed before back end is built, especially before any dependencies.
...
- [ ] 1920px wide
- [ ] 944px high
    - ! not 1080px because of taskbar, browser's bars, et cetera.
-**Gutter:**
...
| 7 | Main Stage | No fixed dimensions | No Scroll in Design; Vertical in Trading; No Main stage in Fly; Vertical scroll in File Converter |

##### Header Bar

- [ ] no header in header bar.
- [ ] `Polly Prediction Partners v0.03.01` text aligned Left, vertically centered.
- [ ] *insert model name and number here*
- [ ] **Illumination Panel Card:** right-aligned, vertically centered.
    - **Illumination Panel:** Overhead Control Panel. Channels, components, elements, and Default states for flip toggles, dimmer dials.
...
- [ ] `Navigation` text aligned left, vertically centered
...
- [ ] **Profit/Loss Line Graph** if no data, display `no data`.
    - [ ] Displays colored lines for each active agent's accurate profit/loss data.
...
| USSR Cockpit Dark    | Night Intercept. region bodies stll (#3d90a2), Radium Green gauges.        |
...
*Only develop the first 3 until User is ready to develop more. Develop one at a time slowly.*

- [ ] Default Modern Webpage
- [ ] Mosaic 1993
- [ ] Office Desk

*Pause here until UI passes version 0.1*
...
*Only develop the first 3 until we pass version 0.1. Develop one at a time slowly.*
...
- [ ] **Position:** Header bar aligned right.
...
- [ ] **Master:** global ON/Off switch; Global DIM Dial
...
- [ ] **Display:** self-illumination intensity for components likedata displays; simulates LCD/LED or MFD displays. Displays are cards or content that are windows, and dropdown menus, graphs, logs Et cetera.
...
- [ ] `00-style-master.css` could hold anything that doesn't naturally belong in a region-specific file.

---

### State Management

- [ ] **CRITICAL:** Initialize CSS variables even in light mode
    - [ ] Don't early-return in functions that set CSS variables
    - [ ] Variables should always have values, even if effects are disabled
- [ ] **CRITICAL:** Handle localStorage migration
    - [ ] Old data format may break new features
...
- [ ] Always put mode/theme-specific rules AFTER base rules
- [ ] Define all custom properties in `:root` upfront
...
- [ ] Backend state — trading data, positions, fills — could never reset with the UI
- [ ] Multiple browser tabs could share one backend — changes in one tab could reflect in both
    - [ ] Opening a new instance of the back end could stop any prior running instance before starting fresh, without erasing trading data.
- [ ] Left Sidebar and Right Sidebar phase out when entering Trading or Flight Studio
    - [ ] Main Region could expand to full width when sidebars are hidden
- [ ] Precision tools (rulers, grid, zero-square) could automatically hide in Trading and Flight studios

---

### Components

*This isn't a full list. just the main components*
...
- [ ] Telemetry
    - [ ] aligned-right, vertically-centered
...
- [ ] Trading Studio State
    - [ ] Live vs Demo Indicator
        - [ ] Live vs Demo mode could have a prominent, impossible-to-miss indicator and light
            - [ ] Sleek initialization animation or brief glow intensity spike on connection
        - [ ] Completely separate data sets depending on which environment is active
    - [ ] Catergories Nav menu
        - [ ] Sub-Categories Nav Menu
            - [ ] Filter options
    - [ ] Series Cards
        - [ ] Expired Market Cards
            - [ ] Expired or settled market cards could grey out in place until a manual refresh is triggered
            - [ ] A series refresh button or market refresh button somewhere in the Trading Studio
        - [ ] The 20 rendered cards could include any cards currently awaiting agent approval
        - [ ] Card Visual Reference
            - [ ] Cards be styled to closely resemble Kalshi's own UI
            - [ ] Reference the Kalshi card photos in the project root for field layout and visual structure
            - [ ] Reference the Kalshi card files in the project filebase for exact field data
...
- [ ] Plan for color blindness (don't rely on color alone)
...
- [ ] How does Fluxe actually make a decision? The strategy is described as "put heart into it" — but what is the first concrete signal it will look at? Spread size? Orderbook imbalance? Recent trade momentum?

A: Idk can you search the internet to see if people have gotten headway on these prediction market trading agents. Fluxe will trade multiple times per minute buying and selling yes's and no's on the 15 min frequency repeating series, or recurring series, for crypto.
...
- [ ] Beyond Flaxe and Fluxe, what would a third agent look like? Is the architecture designed to add agents easily, or would adding a third require significant refactoring?
...
- [ ] The Bottom Bar is `144px` high and needs to fit API key inputs, agent controls, a P/L graph, and a Live/Demo indicator — is that enough vertical space, or does it need a scrollable or expandable design?

A: No scroll. Pack everything creatively. This is a cockpit but it's only 1080p wide and 944 px tall so we gotta be creative. Cockpits are extremely packed with instruments, dials, screens, switches, indicators, lights, sticks, buttons, levers, knobs, and countles more components.
...
- [ ] The 22 theme buttons in the Left Sidebar need to visually represent their own theme regardless of what's currently active — how does a Volcano Dark button look while the Mosaic 1993 Light theme is selected?

A: There are only 22 theme buttons. So If mosiac theme is active, and light mode is selected too, volcano simply looks like volcano light, because there is only one button for each theme. If mosaic theme is active and dark mode is toggled, volcano would look like dark mode.
...
A: They grey out until refresh is hit. Oh we need a series refresh or a market refresh button.
...
A: Only 20 are rendered, including the cards agents need confirmation on.

- [ ] The PRD mentions Live and Demo as separate environments — but is there a UI indicator prominent enough that a user absolutely cannot confuse the two while trading real money?

A: Yes i just updated the prd front end to say this instead
- [ ] **Trading Studio:** `No data` displayed until after succesful API key connection.
    - [ ] Sleek initilization animations glowing animations and/or brief glow intensity spike. 
    - [ ] Live vs Demo mode prominent Indicator and Light.
        - [ ] Completely Different data sets depending on Live or Demo modes initiated.
    - [ ] Category Nav menu, Horizontal scrolling
...
- [ ] Flaxe "follows the majority bets" — but on Kalshi, the orderbook only shows bids. What specific data point defines majority? Best yes bid size? Total open interest? Recent fill direction?
Maybe bets is supposed to say bids

A: We need to think about this. I guess calculate the total majority average buys and sells, and try to follow that as it happens? It's like betting on whatever the majority of that market is guessing because the majority group mind would have the closest answer.
...
A: It's a lot of fields, I have photos with examples for how kalshi's UI organizes and navigates the series cards.

- [ ] There is no defined versioning strategy beyond the `WebPage-0.0.01` label in the Header Bar. When does the version number increment, who decides, and where is the changelog kept?

A: Main user updates version increments with personal judgement. AI Devs 0% worry about version numbers

- [ ] The precision tools — rulers, grid, zero-square — are designed for the Design Studio. Do they stay visible or automatically hide when switching to Trading Studio or Flight Studio?

A: Left side bar and right side bar should phase out and be replaced with a full width main region
...
A: cleared on disconnect, but they are also in the prd, can we make a env. file that the UI can pull and auto fill either the two live keys or the two demo keys?

---

### Even More Open Questions

- [ ] The PRD has 22 themes and 13 architecture palettes — what is the versioning strategy when a theme is updated mid-development? Does changing a token break saved user preferences, and how is that migration handled?

A: Nothing changes to the theme when a design area is changed. Nothing is saved.

- [ ] Flaxe is described as "betting on the majority bets" — but on Kalshi, the best bid/ask spread is the market. What specific orderbook signal defines the majority, and how stale is too stale to act on?

A: they trade what the majority is trading in that minute.

- [ ] The CSS file structure defines `00-style-master.css` but its contents and responsibilities are never described — what goes in it, and what is the rule for deciding if something belongs there versus in a region-specific file?

A: I have no idea i just needed a 00 file
...
A: I'm not sure what this means
...
A: If it is opened it stops all running instances and starts a new one. two browser tabs? hmmmm. it would still have only one back end tho. so i guess I want it like if im in two browsers for a spreadsheet like whatever I do would affect the data in both tabs.

- [ ] The Precision Tools (rulers, grid) are designed for the Design Studio — do they carry over visually into Trading Studio and Flight Studio, or do they hide automatically when the Main Region switches?

A: Yeah let's hide those for the training and flight studios.
